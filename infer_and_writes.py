...
def infer(model_folder, vid_folder, output_folder, model_type="simultaneous"):
    vid_path = Path(vid_folder)
    output_root = Path(output_folder)

    # get video path
    video_files = list(vid_path.rglob("*.mp4"))

    for video_file in video_files:
        output_path = output_root / video_file.stem
        ...
        extract_video(ffmpeg_path, video_file, output_path / "raw_frames")
        ...
        for iid, np_img in enumerate_vid(output_path / "raw_frames"):
            ...
            draw.text(
                (10, 10), f"Count: {counts[iid]}", font=draw_font, fill=(0, 255, 0)
            )
            img.save(count_img_path / f"{iid:06d}.jpg")

        (output_path / "output.mp4").unlink(missing_ok=True)
        create_video(
            ffmpeg_path, output_path / "counts_frames", output_path / "output.mp4"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--video_folder", type=str, help="Path to the video file")
    parser.add_argument("--output_folder", type=str, help="Path to the output folder")
    args = parser.parse_args()
    infer(args.model_folder, args.video_folder, args.output_folder)
